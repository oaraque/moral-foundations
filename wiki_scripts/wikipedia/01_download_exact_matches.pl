...
open my $fh, "<", "conserva_wiki_matches.tsv";
<$fh>;
#Conservapedia title	Conservapedia id	Exact_match	Conserva exact 2 Wiki redirect	Conserva redirects 2 Wiki exact
while (<$fh>) {
	chomp;
	my ($title, $consID, $wikiID, @crap) = split "\t";
	next if $wikiID eq "NULL";
	push @ids_to_download, $wikiID;
}
close $fh;
...
#change this to change how many pages are downloaded per query (max 50)
my $pages_per_batch = 50;
...
while (scalar(@ids_to_download)>0){
	...
	next if -f "exact_matches/$first_pageid.xml"; #avoid redownloading stuff - note: if you changed 35, it might skip too many pages
	my $response = HTTP::Tiny->new->get($url);
	if ($response->{success}) {
		my $json = parse_json ($response->{content});
		my $content = $json->{query}->{export}->{"*"};
		open my $outfh, ">:encoding(UTF-8)", "exact_matches/$first_pageid.xml";
		print $outfh $content;
		close $outfh;
